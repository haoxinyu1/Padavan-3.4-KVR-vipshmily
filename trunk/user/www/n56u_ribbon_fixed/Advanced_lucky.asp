...
<body onload="initial();" onunLoad="return unload_body();">
										...
										<div class="row-fluid">
											...
											<div id="wnd_lucky_cfg">
												<div class="alert alert-info" style="margin: 10px;">
													lucky是一款软硬路由公网神器,集成了多种工具，ipv6/ipv4 端口转发,反向代理,DDNS,WOL,ipv4
													stun内网穿透,cron,acme,阿里云盘,ftp,webdav,filebrowser<br>官网：<a
														href="https://lucky666.cn"
														target="blank">https://lucky666.cn</a><br><br>由于Padavan固件防火墙容易初始化，若lucky里设置自动放行防火墙失效建议手动放行。
													<br>
													<div>当前版本:【<span style="color: #FFFF00;">
															<% nvram_get_x("", "lucky_ver" ); %>
														</span>】&nbsp;&nbsp;最新版本:【<span style="color: #FD0187;">
															<% nvram_get_x("", "lucky_ver_n" ); %>
														</span>】 </div>
												</div>
													...
													<tr>
														...
														<td style="border-top: 0 none;">
															<div class="main_itoggle">
																<div id="lucky_enable_on_of">
																	<input type="checkbox" id="lucky_enable_fake" <%
																		nvram_match_x("", "lucky_enable" , "1"
																		, "value=1 checked" ); %>
																	<% nvram_match_x("", "lucky_enable" , "0"
																		, "value=0" ); %> />
																</div>
															</div>
															<div style="position: absolute; margin-left: -10000px;">
																<input type="radio" value="1" name="lucky_enable"
																	id="lucky_enable_1" class="input" value="1" <%
																	nvram_match_x("", "lucky_enable" , "1" , "checked"
																	); %> /><#checkbox_Yes#>
																	<input type="radio" value="0" name="lucky_enable"
																		id="lucky_enable_0" class="input" value="0" <%
																		nvram_match_x("", "lucky_enable" , "0"
																		, "checked" ); %> /><#checkbox_No#>
															</div>
														</td>
														<td colspan="4" style="border-top: 0 none;">
															<input class="btn btn-success" style="width:150px"
																type="button" name="restartlucky" value="重启"
																onclick="button_restartlucky()" />
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;" colspan="3">
															<div class="input-append">
																<input name="lucky_cmd" type="text" class="input"
																	id="lucky_cmd" placeholder="/etc/storage/lucky"
																	onkeypress="return is_string(this,event);"
																	value="<% nvram_get_x(""," lucky_cmd"); %>"
																size="32" maxlength="128" />
															</div><br><span
																style="color:#888;">⚠️来自官方的友好提醒：<br>经常在软件后台备份配置文件，防止升级或意外丢失配置！</span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;" colspan="3">
															<div class="input-append">
																<input name="lucky_bin" type="text" class="input"
																	id="lucky_bin" placeholder="/tmp/lucky"
																	onkeypress="return is_string(this,event);"
																	value="<% nvram_get_x(""," lucky_bin"); %>"
																size="32" maxlength="128" />
															</div><br><span
																style="color:#888;">自定义程序的存放路径，填写完整的路径和程序名称</span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;">
															<select name="lucky_daji" class="input"
																style="width: 220px;">
																<option value="0" <% nvram_match_x("","lucky_daji", "0"
																	,"selected"); %>>lucky</option>
																<option value="1" <% nvram_match_x("","lucky_daji", "1"
																	,"selected"); %>>lucky.万吉</option>
															</select><br><span
																style="color:#888;">用于在线下载指定程序，万吉比lucky多了FileBrowser模块以及CorazaWAF模块</span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;" colspan="3">
															<div class="input-append">
																<input name="lucky_tag" type="text" class="input"
																	id="lucky_tag" placeholder="<% nvram_get_x("","
																	lucky_ver_n"); %>" onkeypress="return
																is_string(this,event);" value="<%
																	nvram_get_x("","lucky_tag"); %>" size="32"
																	maxlength="128" />
															</div><br><span
																style="color:#888;">用于在线下载时指定lucky版本，防止不同版本配置文件不兼容。</span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;" colspan="3">
															<span
																...
																<code
																	style="color: #000; background-color: #fdd;">nvram set lucky_url="http://your-url/lucky.tar.gz"</code></span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														...
														<td style="border-top: 0 none;" colspan="3">
															<div class="input-append">
																<input name="lucky_extra" type="text" class="input"
																	id="lucky_extra" placeholder="-initFirewall"
																	onkeypress="return is_string(this,event);"
																	value="<% nvram_get_x(""," lucky_extra"); %>"
																size="32" maxlength="128" />
															</div><br><span style="color:#888;">默认启动参数【lucky -cd
																配置文件目录】<br>在上述基础上增加的启动参数，如 -initFirewall</span>
														</td>
													</tr>
													<td colspan="4"></td>
													<tr>
														<th width="30%" style="border-top: 0 none;">管理界面:</th>
														<td style="border-top: 0 none;"><a href="<% nvram_get_x("", "
																lucky_login"); %>" target="_blank"><%
																	nvram_get_x("", "lucky_login" ); %></a>
														</td>
													</tr>
													<td></td>
													<td></td>
	...
	<textarea rows="9" wrap="off" spellcheck="false" maxlength="20240" class="span12" name="scripts.lucky.conf" style="font-family:'Courier New'; font-size:12px;"><% nvram_dump("scripts.lucky.conf",""); %></textarea>
													...
													<tr>
														<td colspan="5"
															style="border-top: 0 none; white-space: nowrap;">
															<div style="display: flex; align-items: center;">
																<span style="margin-right: 10px;">重置为</span>
																<input type="text" id="LuckyCmd" class="span10"
																	name="LuckyCmd" maxlength="127" value=""
																	style="flex: 1; margin-right: 10px;">
																<button class="btn span2" onClick="clearOut();"
																	type="button" value="<#CTL_refresh#>" name="action"
																	style="outline: 0;">清空输入框</button>
															</div>
														</td>
													...
													<tr>
														<td colspan="5" style="border-top: 0 none; text-align: center;">
															<div
																...
																<input class="btn btn-success" id="btn_exec1"
																	onClick="userPost()" type="button" value="重置用户名"
																	name="action" style="width: 15%;">
																<input class="btn btn-success" id="btn_exec2"
																	onClick="passPost()" type="button" value="重置密码"
																	name="action" style="width: 15%;">
																<input class="btn btn-success" id="btn_exec3"
																	onClick="portPost()" type="button" value="重置访问端口"
																	name="action" style="width: 15%;">
																<input class="btn btn-success" id="btn_exec4"
																	onClick="safePost()" type="button" value="重置安全入口"
																	name="action" style="width: 15%;">
																<input class="btn btn-success" id="btn_exec5"
																	onClick="Internettrue()" type="button"
																	value="启用外网访问" name="action" style="width: 15%;">
																<input class="btn btn-success" id="btn_exec6"
																	onClick="Internetfalse()" type="button"
																	value="禁用外网访问" name="action" style="width: 15%;">
															</div>
														</td>
													</tr>
										...
										<table width="100%" cellpadding="4" cellspacing="0" class="table">
											...
											<tr>
												<td width="15%" style="text-align: left; padding-bottom: 0px;">
													<input type="button" onClick="location.reload()" value="刷新日志"
														class="btn btn-primary" style="width: 200px">
												</td>
												<td width="75%" style="text-align: right; padding-bottom: 0px;">
													<input type="button" onClick="clearLog();" value="清除日志"
